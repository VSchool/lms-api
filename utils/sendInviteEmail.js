...
async function sendInviteEmail(invitedUser) {
    try {
        ...
        if (invitedUser.admin) {
            ...
            html = `
                            <div style="text-align: center">
                                ...
                                <a href=${process.env.ADMIN_SIGNUP_URL + "?token=" + token + "&email=" + invitedUser.email + "&firstName=" + invitedUser.name.first + "&lastName=" + invitedUser.name.last }>Click here to sign up as an admin.</a>
                            </div>
                            `
        } else {
            ...
            html = `
                            <div style="text-align: center">
                                ...
                                <p>Welcome to the ${invitedUser.course.name} course at V School, ${invitedUser.name.first}! Here's your invite to join MantisLMS, V School's learning management system.</p>
                                ...
                                <a href=${process.env.STUDENT_SIGNUP_URL + "?token=" + token + "&email=" + invitedUser.email + "&firstName=" + invitedUser.name.first + "&lastName=" + invitedUser.name.last + "&courseId=" + invitedUser.course._id}>Click here to sign up for MantisLMS.</a>
                            </div>
                            `
        }
        ...
        return { statusCode: 200, message: messageUrl }
    } catch (e) {
